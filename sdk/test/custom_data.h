...
#define CUSTOM_DATA_ACTUAL_SIZE 0       // 实际数据长度
 ...
 * @brief 纯数据结构（不含图片块）
 * @note 用于无图片传输场景，节省内存
 * @size 1 Bytes (1B类型 + 0B数据)
 */
#pragma pack(push, 1)
typedef struct {
    uint8_t packet_type; // 0x00: 纯数据
    int8_t drtfgyhujikolp;;
    int8_t huhjjhjhjhhjhhhh;
} CustomData_t;
#pragma pack(pop)
 ...
 * @size 129 Bytes (1B类型 + 0B伴随数据 + 128B图片)
 */
#pragma pack(push, 1)
typedef struct {
    uint8_t packet_type; // 0x01: 含图片数据
    ImageBlock_t d; // 图片块 (128B)
